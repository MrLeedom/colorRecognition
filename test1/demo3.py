...
class Detect:
    def __init__(self, path):
        ...
        print(cols)
        # 工作图像
        self.work_img = cv2.resize(self.ori_img, (int(cols / 4),int(rows / 4)))
        self.work_gray = cv2.resize(self.gray, (int(cols / 4), int(rows / 4)))
        self.work_hsv = cv2.resize(self.hsv, (int(cols / 4), int(rows / 4)))
    ...
    def key_points_tap(self, img):
        ...
        cnts, _ = cv2.findContours(
            img_cp, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        # 按区域大小排序,找到第二大轮廓
...
if __name__ == '__main__':
    ...
    while True:
        img_index += 1
        img_path = root_path + '/' + str(23420) + '.png'
        d = Detect(img_path)
        d.img_process_main()