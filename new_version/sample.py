...
class Detect:
    def __init__(self, path):       
        ...
        self.work_img = cv2.resize(
            self.ori_img, (int(cols/2), int(rows/2)))
        self.work_gray = cv2.resize(
            self.gray, (int(cols/2), int(rows/2)))
        self.work_hsv = cv2.resize(
            self.hsv, (int(cols/2), int(rows/2)))
    ...
    def img_process_main(self,count):
        ...
        if flag == 2:
            ...
            if red_width > 2*blue_width :
                ...
                key_length,key_angle = self.get_Angle_Distance(blue_width+red_width,blue_distance[0]+red_distance[0],key_position)
                # print('key_length:',key_length)
                # print('key_angle:',key_angle)
